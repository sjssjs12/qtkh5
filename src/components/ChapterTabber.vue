  ...
  <div v-if="qTabberConf.showTabber">
    <div class="tabbar-wrap">
      <div class="item-tabber">
        <div class="item-tabber-icon toggleColor"  @click="toggleFavourite"><b :class="like"></b></div>
        <p class="item-tabber-label" :class="{'toggleColor' : this.favourite}">收藏</p>
      </div>
      <div class="item-tabber" @click="showAnswerSheet" v-if="qTabberConf.showAnswerSheetBtn">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('uiModule', ['qTabberConf']),
      ...mapGetters('qModule', ['removeCollection', 'baseInfo', 'curQuestionID', 'answerSheet',
    ...
    methods: {
      ...
      removeWrong () {
        if (!process.env.HTML_Debug) {
          errorAPI.anserSheetErrorRemove(this.curQuestionID, this.baseInfo.userid).then(({data}) => {})
        }
        this.refresh()
      },
      doPost (questionid, value) {
        ...
        if (!process.env.HTML_Debug) {
          if (value) {
            collectionAPI.collectionAdd(this.baseInfo.userid, this.baseInfo.productid, questionid, this.baseInfo.subsubjectid, 1).then(({data}) => {
            })
          } else {
